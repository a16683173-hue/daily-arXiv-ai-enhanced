...
Method: 1) 将2D参考图像提升为多视角表示；2) 采用双路径视角一致条件机制；3) 质量感知加权机制处理噪声输入；4) 集成感知一致性模块解决遮挡和边界伪影

Result: 实验结果表明该方法显著提升视频对象插入质量，提供稳定且逼真的集成效果
...
Method: 挑战赛采用双赛道设计：1）恢复赛道强调像素级保真度，使用PSNR作为评估指标；2）感知赛道关注视觉真实感，使用感知分数进行评估。数据集通过双三次下采样生成×4倍的低分辨率图像。

Result: 共有194名参与者注册，31支队伍提交了有效方案。挑战赛总结了参与团队的主要方法、结果和性能表现。

Conclusion: NTIRE 2026超分辨率挑战赛为图像超分辨率领域提供了统一的基准测试，展示了当前进展并为未来研究方向提供了见解。
...
TL;DR: DVFace是一个用于真实世界视频人脸恢复的一步扩散框架，通过时空双码本设计和不对称融合模块，在保持高质量恢复的同时实现高效推理。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的方法主要依赖通用扩散先验和多步采样，限制了面部适应性和推理效率。需要一种既能实现忠实面部恢复又能保持时间稳定性的高效方法。

Method: 提出DVFace框架：1）时空双码本设计，从退化视频中提取互补的空间和时间面部先验；2）不对称时空融合模块，根据先验的不同作用将其注入扩散主干网络。

Result: 在多个基准测试中，DVFace相比现有方法在恢复质量、时间一致性和身份保持方面表现更优。

Conclusion: DVFace通过一步扩散框架和创新的时空先验提取融合机制，实现了高效且高质量的视频人脸恢复，在真实性和稳定性方面均有显著提升。
...
Method: 提出MapSR框架：1）使用低分辨率标签通过轻量级线性探针从冻结的视觉基础模型特征中提取类别提示；2）通过免训练度量推理和基于图的预测细化进行高分辨率制图；3）通过余弦相似度匹配和基于图的传播进行空间细化。

Result: 在Chesapeake Bay数据集上，MapSR在没有任何高分辨率标签的情况下达到59.64% mIoU，与最强的弱监督基线竞争力相当，并超越了完全监督基线。可训练参数减少四个数量级，训练时间从小时级缩短到分钟级。
...
TL;DR: 提出Seen-to-Scene框架，统一传播和生成范式，通过流补全网络和参考引导潜传播实现高质量视频外绘，解决现有方法的时间不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于生成模型的方法存在隐式时间建模和有限空间上下文的问题，导致帧内和帧间不一致，在动态场景和大范围外绘时尤为明显。

Method: 提出统一传播和生成范式的框架：1) 使用为视频修复预训练的流补全网络进行流传播，端到端微调以弥合领域差距；2) 引入参考引导潜传播，有效跨帧传播源内容。

Result: 实验表明该方法在时间一致性和视觉真实感方面优于现有方法，推理效率高，甚至超过需要输入特定适应的先前最先进方法。

Conclusion: Seen-to-Scene框架通过统一传播和生成范式，有效解决了视频外绘中的时间一致性问题，实现了高质量、高效率的视频内容扩展。
...
TL;DR: 本文提出使用图像到视频生成模型创建逼真的指点手势合成数据集，以解决手势识别领域的数据稀缺问题，并通过实验证明合成数据能有效增强下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 手势识别研究面临严重的数据稀缺问题，传统方法依赖昂贵的人工录制或无法生成真实手势变化的图像处理方法。图像到视频基础模型的最新进展为创建无需人工干预的合成数据提供了新可能。

Method: 提出基于提示的视频生成方法构建逼真的指点手势数据集，设计数据生成流程从少量人类参与者参考样本生成指点手势，创建可被机器学习社区内外利用的易用方法。

Result: 合成手势在视觉保真度上与真实手势高度一致，同时引入了有意义的变化和新颖性，丰富了原始数据。使用混合数据集的各种深度模型表现出更优越的性能。

Conclusion: 图像到视频技术即使在早期阶段，也为手势合成提供了强大的零样本方法，对下游任务有明显益处，为解决手势识别数据稀缺问题提供了有效途径。
...
TL;DR: Flow of Truth是首个专注于图像到视频生成中时间维度取证的主动框架，通过像素运动追踪而非帧合成来应对I2V内容随时间演变的取证挑战。


<details>
  <summary>Details</summary>
Motivation: 随着图像到视频生成技术的快速发展，需要新的取证方法来应对视频内容随时间演变的特性。传统2D像素级篡改定位方法在处理视频时效果有限，因为嵌入的痕迹会随着帧的推移而漂移和变形。

Method: 将视频生成重新定义为"像素随时间运动而非帧合成"，提出可学习的取证模板来跟随像素运动，以及模板引导的流模块来解耦运动与图像内容，实现鲁棒的时间追踪。

Result: 实验表明Flow of Truth能够泛化到商业和开源I2V模型，显著提升了时间维度取证性能。

Conclusion: 该研究首次解决了I2V生成中的时间维度取证问题，通过像素运动追踪的新视角为视频取证提供了有效解决方案。
...
TL;DR: PIE-V是一个通过心理学启发的错误注入框架，用于构建和评估包含人为错误的自我中心视角程序视频，通过可控的、人类可信的偏差来增强干净的关键步骤程序。


<details>
  <summary>Details</summary>
Motivation: 可靠的视频程序监控需要暴露自然发生的人为错误及其恢复过程。在自我中心视角录像中，错误常被手部部分遮挡，通过微妙的物体状态变化显现，而现有程序数据集提供的错误和修正痕迹有限且不一致。

Method: PIE-V框架包含：1) 基于程序阶段和语义步骤负载的心理学启发错误规划器；2) 建模恢复行为的修正规划器；3) 执行级联一致重写的LLM编写器；4) 验证程序连贯性和修复失败的LLM判断器。对于视频片段编辑，使用文本引导的视频生成合成替换片段并缝合到原视频中。

Result: 应用于17个任务和50个Ego-Exo4D场景，PIE-V注入了102个错误并生成了27个恢复修正。引入统一分类法和包含9个指标的人类评估标准，涵盖步骤级和程序级质量。与自由形式LLM生成基线相比，PIE-V表现更优。

Conclusion: PIE-V框架和评估标准支持自我中心视角程序错误检测和修正的后完成验证，为构建包含可信人为错误的程序视频提供了系统方法。
...
TL;DR: 提出首个系统性评估动画图像到视频生成的基准AnimationBench，针对动画风格特性设计评估维度，支持标准化和灵活评估，比现有真实视频基准更准确反映动画生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成基准主要针对真实视频设计，难以评估动画风格生成的质量。动画具有风格化外观、夸张动作和角色一致性等特点，现有基准使用固定提示词和刚性流程，缺乏对开放领域内容和定制评估的灵活性。

Method: 将动画十二原则和IP保护转化为可测量的评估维度，包括语义一致性、动作合理性、相机运动一致性等更广泛的质量维度。支持标准化封闭集评估和灵活开放集评估，利用视觉语言模型进行可扩展评估。

Result: 实验表明AnimationBench与人类判断高度一致，能揭示被真实视频基准忽视的动画特定质量差异，对最先进的I2V模型提供更具信息量和区分度的评估。

Conclusion: AnimationBench填补了动画视频生成评估的空白，通过系统化评估动画特定特性，为动画图像到视频生成提供了更准确、全面的评估框架。
