...
Motivation: 文本驱动视频生成已普及，但在电影多镜头场景中的相机控制仍面临挑战：隐式文本提示不精确，显式轨迹标注既繁琐又容易导致模型执行失败，需要更高效精确的解决方案。

Method: 提出数据中心的范式转变，构建对齐的(字幕、轨迹、视频)三元组联合分布。采用"规划-控制"框架：1) VLM规划器利用空间先验从文本生成电影级全局对齐轨迹；2) 控制器通过相机适配器将轨迹渲染为多镜头视频。关键创新是自动多镜头相机校准流程，将分散的单镜头轨迹对齐到统一坐标系。

Result: 构建了ShotVerse-Bench高质量电影数据集和三轨评估协议。实验表明，ShotVerse有效弥合了不可靠文本控制和繁琐手动规划之间的差距，在相机精度和跨镜头一致性方面表现优异，实现了卓越的电影美学效果。

Conclusion: ShotVerse通过数据中心的"规划-控制"范式，成功解决了多镜头视频生成中的相机控制瓶颈，实现了从文本到精确相机轨迹再到高质量视频的端到端生成，为电影制作提供了高效可靠的自动化工具。
...
Method: 提出多帧节点引导框架，使用时间Transformer在短帧窗口内建模运动依赖；采用稀疏控制节点提高效率，每个节点包含解耦的规范位置和潜在编码；通过输入掩码策略和两种多帧损失进行端到端训练

Result: 在广泛实验中，Mango-GS实现了最先进的重建质量和实时渲染速度，能够高保真重建和交互式渲染动态场景

Conclusion: Mango-GS通过多帧节点引导方法有效解决了动态3D场景重建中的时间一致性问题，在保持高效率的同时实现了高质量的重建效果
...
TL;DR: UCAN是一个轻量级超分辨率网络，统一了卷积和注意力机制，通过Hedgehog注意力、蒸馏式大核模块和跨层参数共享，在保持高性能的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 混合CNN-Transformer架构在图像超分辨率中表现优异，但扩大注意力窗口或卷积核会显著增加计算成本，限制了在资源受限设备上的部署。需要设计更高效的架构来扩展有效感受野。

Method: 1) 结合窗口空间注意力和Hedgehog注意力机制，同时建模局部纹理和长程依赖；2) 引入基于蒸馏的大核模块，在不增加计算负担的情况下保持高频结构；3) 采用跨层参数共享进一步降低复杂度。

Result: 在Manga109数据集上（4倍超分），UCAN-L达到31.63 dB PSNR，仅需48.4G MACs，超越近期轻量级模型。在BSDS100上达到27.79 dB，优于参数规模更大的方法。实验表明UCAN在精度、效率和可扩展性之间取得了优越的平衡。

Conclusion: UCAN通过统一卷积和注意力机制，实现了高效的感受野扩展，在保持高性能的同时大幅降低计算复杂度，非常适合实际高分辨率图像恢复应用。
...
TL;DR: COTONET是一个基于YOLO11改进的棉花蒴果检测模型，通过引入多种注意力机制和架构改进，在保持轻量化的同时实现了优于基准模型的检测性能。


<details>
  <summary>Details</summary>
Motivation: 棉花采摘过程中物理操作可能导致纤维质量下降，需要模仿人工轻柔抓取。自动化采摘需要能够识别不同生长阶段棉花蒴果的系统，但现有模型在检测困难实例方面存在不足。

Method: 提出COTONET模型，基于YOLO11架构进行改进：1) 用Squeeze-and-Excitation块替换卷积块；2) 重新设计主干网络集成注意力机制；3) 用CARAFE替换标准上采样操作；4) 集成SimAM进行主要特征聚合；5) 在向下路径中使用PHAM进行通道、空间和坐标注意力。

Result: COTONET在保持轻量化（7.6M参数，27.8 GFLOPS）的同时，实现了mAP50为81.1%，mAP50-95为60.6%，优于标准YOLO基线模型。

Conclusion: COTONET通过集成多种注意力机制改进了棉花蒴果检测，特别适合低资源边缘计算和移动机器人应用，为自动化棉花采摘提供了有效的视觉识别解决方案。
...
TL;DR: 本文提出Neighbor Forcing和ConvKV记忆机制，解决了自回归扩散模型在实时人体动画中的训练不稳定和推理效率问题，实现了小时级实时生成。
...
Method: 提出Neighbor Forcing方法，这是一种扩散步骤一致的自回归公式，在相同噪声条件下传播时间相邻帧作为潜在邻居，提供分布对齐的稳定学习信号。在此基础上引入结构化ConvKV记忆机制，将因果注意力中的键值压缩为固定长度表示，实现恒定内存推理和真正无限视频生成。

Result: LiveAct方法显著改善了训练收敛性、小时级生成质量和推理效率，在仅使用两个NVIDIA H100或H200 GPU的情况下，支持20 FPS实时流式推理，在唇形同步准确性、人体动画质量和情感表达方面达到最先进性能，且推理成本最低。

Conclusion: 通过Neighbor Forcing和ConvKV记忆机制，成功解决了自回归扩散模型在实时人体动画中的扩展性问题，实现了高效、高质量的小时级实时生成，为序列生成任务提供了有效的解决方案。
...
TL;DR: 提出新框架，使用稀疏3D手部关节作为控制信号，生成第一人称视角视频，解决遮挡问题并实现跨具身泛化到机械手。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成第一人称视角视频时，常因使用2D轨迹或隐式姿态而难以实现3D一致的手部精细动作，导致运动不一致、伪影，且无法泛化到机械手。

Method: 提出新框架：1) 使用稀疏3D手部关节作为控制信号；2) 引入高效控制模块，通过惩罚隐藏关节的不可靠信号提取遮挡感知特征，使用3D加权机制处理动态遮挡；3) 将3D几何嵌入直接注入潜在空间以保持结构一致性；4) 开发自动标注流程生成百万级高质量视频-轨迹对；5) 构建跨具身基准。

Result: 方法显著优于现有基线，能生成高保真第一人称视频，实现逼真交互，并展现出对机械手的卓越跨具身泛化能力。

Conclusion: 提出的框架通过使用3D手部关节作为控制信号和创新的遮挡处理机制，成功解决了第一人称视频生成中的3D一致性和跨具身泛化问题。
...
TL;DR: HomeSafe-Bench：首个专注于家庭场景不安全动作检测的基准测试，包含438个多样化案例；HD-Guard：分层流式架构，通过轻量FastBrain和高频筛查与异步大型SlowBrain深度推理的协同，实现实时安全监控。


<details>
  <summary>Details</summary>
Motivation: 家庭环境不同于结构化工业场景，存在不可预测的安全风险，如感知延迟和常识知识缺乏可能导致危险错误。现有安全评估多局限于静态图像、文本或一般危险，无法充分评估动态不安全动作检测。

Method: 1) 构建HomeSafe-Bench基准：通过物理仿真与先进视频生成的混合流程创建，包含438个多样化案例，覆盖六个功能区域，具有细粒度多维标注；2) 提出HD-Guard架构：分层流式架构，协调轻量级FastBrain进行连续高频筛查，与异步大型SlowBrain进行深度多模态推理。

Result: HD-Guard在延迟与性能之间实现了优越的权衡，评估显示其有效性；分析揭示了当前基于VLM的安全检测中的关键瓶颈。

Conclusion: HomeSafe-Bench填补了家庭场景不安全动作检测评估的空白，HD-Guard架构为实时安全监控提供了有效的解决方案，平衡了推理效率与检测精度，有助于推动家庭机器人安全性的提升。
...
TL;DR: 提出一种基于h变换的训练自由引导生成方法，通过修改采样过程的转移概率来引导生成高质量视觉样本，无需知道前向变换算子。


<details>
  <summary>Details</summary>
Motivation: 粗引导视觉生成在现实应用中很重要，但基于训练的方法成本高、泛化受限，而现有的训练自由方法要么需要知道前向变换算子，要么难以平衡引导与合成质量。

Method: 使用h变换约束随机过程，通过向原始微分方程添加漂移函数来修改每个采样时间步的转移概率，引导生成朝向理想样本。引入噪声水平感知调度来减少近似误差影响。
...
Result: 在两个适配器架构上的实验表明，FlashMotion在视觉质量和轨迹一致性方面都超越了现有视频蒸馏方法和之前的多步模型。还引入了FlashBench基准，用于评估长序列轨迹可控视频生成。
...
TL;DR: 提出两阶段双模态模型用于ABAW竞赛的表情识别，结合视觉DINOv2特征和音频Wav2Vec特征，通过门控融合和时间平滑提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决无约束视频中表情识别的挑战：人脸定位不准确、姿态和尺度变化大、运动模糊、时间不稳定等干扰因素。

Method: 两阶段双模态方法：第一阶段使用DINOv2 ViT-L/14提取视觉特征，采用PadAug数据增强和MoE训练头；第二阶段多尺度重裁剪人脸，结合Wav2Vec音频特征，通过门控融合模块和推理时时间平滑进行双模态融合。

Result: 在ABAW数据集上，验证集Macro-F1达到0.5368，5折交叉验证为0.5122±0.0277，优于官方基线。

Conclusion: 提出的两阶段双模态模型能有效处理无约束视频中的表情识别挑战，通过视觉特征增强和音频融合提升了识别性能。
...
TL;DR: DreamVideo-Omni：一个统一框架，通过两阶段训练实现多主体定制和全方位运动控制，解决身份保持和运动控制的挑战。
...
Method: 采用渐进式两阶段训练范式：第一阶段整合全面控制信号进行联合训练，引入条件感知3D旋转位置编码和分层运动注入策略；第二阶段通过潜在身份奖励反馈学习缓解身份退化。

Result: 在构建的大规模数据集和DreamOmni基准测试中，DreamVideo-Omni在生成高质量视频和精确控制方面表现出优越性能。

Conclusion: 该框架成功实现了多主体和谐定制与全方位运动控制，在身份保持和运动控制精度方面取得显著进展。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频分词器对所有视频采用均匀的token分配策略，导致简单、静态或重复的视频片段浪费token，而动态或复杂的片段token不足。这种低效分配影响了重建质量和下游生成的计算成本平衡。
...
Conclusion: EVATok通过自适应token分配策略有效解决了传统视频分词器的低效问题，在保持高质量重建的同时显著降低了计算成本，为视频生成任务提供了更高效的分词解决方案。
