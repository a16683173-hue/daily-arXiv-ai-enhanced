...
TL;DR: 提出sFRC方法，通过在小块上执行傅里叶环相关分析来检测深度学习医学影像重建中的幻觉伪影


<details>
  <summary>Details</summary>
Motivation: 深度学习在医学影像重建中会产生视觉上吸引人但可能包含幻觉伪影的结果，目前缺乏易用的幻觉检测技术和鲁棒指标

Method: 提出sFRC方法：在小图像块上执行傅里叶环相关分析，并扫描比较DL输出与参考图像，可基于专家标注或成像理论设置参数

Result: sFRC能有效检测CT超分辨率、CT稀疏视图和MRI欠采样重建中的幻觉特征，与成像理论结果一致，并能量化DL方法在不同数据分布和采样率下的幻觉率

Conclusion: sFRC是一种有效的幻觉检测工具，不仅适用于DL方法，也适用于传统正则化方法和先进展开方法，有助于评估医学影像重建方法的鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 真实世界红外图像超分辨率是一个实际重要但很少被研究的任务。现有方法通常在模拟数据集上训练评估，或忽略了红外与可见光成像的本质差异。实际红外图像受到光学和传感退化的耦合影响，同时损害结构清晰度和热保真度。

Method: 提出Real-IISR统一自回归框架，通过热结构引导视觉自回归逐步重建细粒度热结构和清晰背景。包含热结构引导模块、条件自适应码本和热序一致性损失。

Result: 构建了FLIR-IISR真实世界红外超分辨率数据集，实验证明Real-IISR在真实世界红外超分辨率任务上表现出色，为领域提供了统一基础和基准。

Conclusion: Real-IISR为解决真实世界红外图像超分辨率问题提供了一个有效的统一框架，通过热结构引导的自回归方法和物理一致性约束，显著提升了红外图像的结构清晰度和热保真度。
...
Method: 1. 多保真度纹理学习模块：整合微调的DSA专用超分辨率模型的高质量先验到4D重建优化中；2. 置信感知策略：自适应加权低分辨率投影和生成的高分辨率伪标签之间的监督信号；3. 辐射亚像素致密化：利用高分辨率亚像素采样的梯度累积来细化4D辐射高斯核。

Result: 在两个临床DSA数据集上的广泛实验表明，DSA-SRGS在定量指标和定性视觉保真度方面显著优于现有最先进方法。

Conclusion: DSA-SRGS成功解决了动态稀疏视图DSA重建中的超分辨率问题，能够恢复细粒度血管细节，为精准脑血管疾病诊断和治疗提供了更好的4D血管模型。
...
TL;DR: FC-VFI：基于预训练视频扩散模型的视频帧插值方法，通过潜在序列时序建模和语义匹配线实现高保真度和运动一致性，支持4×和8×插值
...
Result: FC-VFI支持4×和8×插值，可将30 FPS提升至120和240 FPS（2560×1440分辨率），在多种场景下保持高保真度和运动一致性，实验显示其具有高性能和结构完整性

Conclusion: FC-VFI通过创新的时序建模、语义匹配线引导和时序差异损失，解决了视频帧插值中的保真度和一致性问题，实现了高质量的视频帧率提升
...
<details>
  <summary>Details</summary>
Motivation: 当前4D合成研究面临缺乏大规模4D数据集的问题，导致模型无法充分学习高质量的4D生成所需的关键时空特征，阻碍了该领域的发展

Method: 提出STD-4D扩散模型，通过解耦的时空潜在变量合成4D感知视频；设计正交时空分布转移机制，将3D扩散模型的空间先验和视频扩散模型的时间先验迁移到4D合成；开发时空感知的HexPlane来整合转移的时空特征

Result: 实验表明该方法显著优于现有方法，实现了更好的时空一致性和更高质量的4D合成

Conclusion: 通过迁移现有3D和视频扩散模型的丰富先验知识，可以有效解决4D数据集缺乏的问题，推动高质量4D内容生成的发展
...
TL;DR: GEM-TFL：一种基于图模型和EM优化的弱监督时序伪造定位方法，通过两阶段分类-回归框架、多维度潜在属性增强、时序一致性优化和图基提案精炼，显著提升了弱监督时序伪造定位的性能。
...
Method: 提出GEM-TFL两阶段分类-回归框架，包括：1）通过EM优化将二元标签重构为多维度潜在属性以增强弱监督；2）训练免费的时序一致性优化使帧级预测更平滑；3）图基提案精炼模块建模提案间的时序-语义关系以实现全局一致的置信度估计。

Result: 在基准数据集上的大量实验表明，GEM-TFL实现了更准确和鲁棒的时序伪造定位，显著缩小了与全监督方法的差距。

Conclusion: GEM-TFL通过创新的两阶段框架、EM增强的弱监督、时序一致性优化和图基关系建模，有效解决了弱监督时序伪造定位的关键挑战，为多媒体取证和安全提供了更实用的解决方案。
...
TL;DR: SpectralCache通过识别DiT去噪过程中的三个非均匀性维度（时间、深度、特征），提出统一的缓存框架，在保持生成质量的同时实现2.46倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有DiT缓存方法将去噪过程视为均匀的，忽略了时间、深度和特征维度的非均匀性，限制了加速效果。
...
Conclusion: SpectralCache是无需训练、即插即用的统一缓存框架，通过识别并利用DiT去噪的非均匀性，显著提升推理速度同时保持生成质量。
...
TL;DR: EdgeDAM：一种轻量级检测引导的跟踪框架，在边缘设备上实现抗干扰的单目标跟踪，通过双缓冲干扰感知内存和置信度驱动切换机制，在保持实时性能的同时提高对遮挡和快速运动的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前基于分割的干扰感知内存机制计算开销大，难以在资源受限的边缘设备上实时部署；而轻量级跟踪器在面对视觉相似干扰物时容易漂移。需要在边缘设备上实现既准确又实时的单目标跟踪。

Method: 提出EdgeDAM框架：1）双缓冲干扰感知内存（DAM），包括保留时间一致目标假设的近期感知内存和显式存储硬负样本的干扰解析内存；2）置信度驱动切换与保持框稳定机制，根据跟踪器可靠性和时间一致性标准自适应激活检测和内存引导的重新识别。

Result: 在包括DiDi数据集在内的五个基准测试中表现出色，在遮挡和快速运动下具有改进的鲁棒性，在iPhone 15上达到25 FPS的实时性能，在DiDi数据集上达到88.2%的准确率。

Conclusion: EdgeDAM成功解决了边缘设备上单目标跟踪的准确性与实时性平衡问题，通过重新设计干扰感知内存机制，在保持轻量级的同时显著提高了对干扰物和遮挡的鲁棒性，为资源受限环境下的实时跟踪提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的视频生成方法虽然质量高，但运行速度慢，主要瓶颈在于时空注意力机制的计算开销过大。研究发现许多token连接在不同输入下都产生可忽略的分数，且模式具有重复性，这为加速提供了机会。

Method: CalibAtt通过离线校准步骤识别跨输入稳定的块级稀疏性和重复模式，将这些模式编译为针对每层、每个注意力头和扩散时间步的优化注意力操作。推理时只计算选中的输入相关连接，以硬件高效的方式跳过未选中的连接。

Result: 在Wan 2.1 14B、Mochi 1和各种分辨率的多步蒸馏模型上的实验表明，CalibAtt实现了最高1.58倍的端到端加速，优于现有的无需训练方法，同时保持了视频生成质量和文本-视频对齐。

Conclusion: CalibAtt通过利用注意力机制中的稳定稀疏模式，为视频生成提供了高效的无训练加速方案，在保持质量的同时显著提升推理速度，为实际应用提供了实用解决方案。
...
Method: 提出面部定制的尺度感知相机变换表示，无需3D先验；训练视频生成模型时结合多视角工作室捕捉和野外单目视频；引入合成相机运动和多重拍摄拼接两种相机控制数据生成策略

Result: 在Ava-256数据集和多样野外视频上的实验表明，FaceCam在相机可控性、视觉质量、身份和运动保持方面表现优异

Conclusion: FaceCam通过面部定制的尺度感知相机表示和创新的数据生成策略，成功实现了高质量、可控的人像视频相机轨迹生成
