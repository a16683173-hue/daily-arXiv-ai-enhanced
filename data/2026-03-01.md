...
TL;DR: DPCache：一种基于动态规划的扩散模型加速框架，将采样加速视为全局路径规划问题，通过最小化路径相关误差来选择最优关键时间步序列，实现高质量加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像和视频生成中表现出色，但多步迭代采样带来的计算开销限制了实际部署。现有的基于缓存的加速方法采用固定或局部自适应调度，未考虑去噪轨迹的全局结构，容易导致误差累积和视觉伪影。

Method: DPCache将扩散采样加速构建为全局路径规划问题。首先从小型校准集构建路径感知成本张量，量化在给定前一个关键时间步条件下跳过时间步的路径相关误差。然后使用动态规划选择最小化总路径成本的关键时间步最优序列。推理时仅在关键时间步进行完整计算，中间输出使用缓存特征高效预测。

Result: 在DiT、FLUX和HunyuanVideo上的实验表明，DPCache在保持最小质量损失的情况下实现了显著加速。在FLUX上，4.87倍加速时ImageReward提升+0.031，3.54倍加速时甚至超过完整步数基线+0.028 ImageReward，优于现有加速方法。

Conclusion: DPCache通过路径感知的全局调度框架，有效解决了扩散模型加速中的误差累积问题，实现了高质量的训练免费加速，为扩散模型的实际部署提供了有前景的解决方案。
...
TL;DR: SPATIALALIGN：一个通过零阶正则化DPO微调T2V模型的自改进框架，提升模型对文本提示中动态空间关系的表达能力


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成器注重美学质量，但往往忽略生成视频中的空间约束，无法准确表达文本提示中的动态空间关系

Method: 1. 提出SPATIALALIGN自改进框架；2. 设计零阶正则化直接偏好优化方法微调T2V模型；3. 开发基于几何的DSR-SCORE度量标准；4. 构建包含多样动态空间关系的文本-视频对数据集

Result: 微调后的模型在空间关系表达上显著优于基线模型，DSR-SCORE提供了比依赖VLM评估更精确的量化测量方法

Conclusion: SPATIALALIGN框架有效提升了T2V模型对动态空间关系的表达能力，为空间约束的视频生成提供了新的解决方案
...
Method: 提出UCM框架：1）时间感知位置编码扭曲机制统一长期记忆和相机控制；2）高效双流扩散transformer降低计算开销；3）基于点云渲染的可扩展数据策展策略，在50万+单目视频上训练。

Result: 在真实世界和合成基准测试中，UCM在长期场景一致性方面显著优于最先进方法，同时在高质量视频生成中实现了精确的相机可控性。

Conclusion: UCM通过统一长期记忆和相机控制的创新框架，解决了视频生成世界模型中的关键挑战，为交互环境模拟提供了更强大和可控的解决方案。
...
TL;DR: 提出无需配对数据的在线视频稳定新框架，采用三阶段经典流程与多线程缓冲机制，解决端到端学习的数据限制、可控性差和硬件效率问题，并在新无人机数据集上超越现有在线方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的视频稳定方法需要配对数据集（稳定与不稳定视频），这限制了数据获取。端到端学习方法存在三个长期挑战：数据有限、可控性差、在资源受限硬件上效率低下。此外，现有基准测试主要关注手持设备的前向可见光视频，限制了在无人机夜间遥感等领域的应用。

Method: 提出无监督在线视频稳定框架，采用经典三阶段稳定流程（运动估计、运动平滑、帧合成），并引入多线程缓冲机制。该方法无需配对训练数据，通过模块化设计提高可控性，优化硬件资源使用效率。

Result: 在新提出的多模态无人机航拍视频数据集（UAV-Test）上进行实验，在定量指标和视觉质量上均优于现有最先进的在线稳定方法，性能接近离线方法水平。

Conclusion: 该无监督在线视频稳定框架有效解决了端到端学习方法的三大挑战，无需配对数据集，在无人机遥感等新领域表现出色，为资源受限环境下的视频稳定提供了实用解决方案。
...
TL;DR: Uni-Animator是一个基于Diffusion Transformer的统一图像和视频草图着色框架，解决了现有方法在颜色转移、细节保留和时间一致性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有草图着色方法难以统一图像和视频任务，存在颜色转移不精确、高频物理细节保留不足、大运动场景中时间一致性差等问题。

Method: 提出三个关键技术：1) 通过实例块嵌入实现视觉参考增强，精确对齐和融合参考颜色信息；2) 使用物理特征进行物理细节强化，有效捕捉和保留高频纹理；3) 基于草图的动态RoPE编码，自适应建模运动感知的时空依赖关系。

Result: 实验结果表明，Uni-Animator在图像和视频草图着色上都取得了有竞争力的性能，与任务特定方法相当，同时实现了统一的跨域能力，具有高细节保真度和鲁棒的时间一致性。

Conclusion: Uni-Animator成功解决了草图着色中图像和视频任务的统一问题，在颜色转移精度、细节保留和时间一致性方面表现出色，为跨域草图着色提供了有效的解决方案。
...
TL;DR: ColoDiff是一个基于扩散模型的结肠镜视频生成框架，通过时间流模块和内容感知模块实现动态一致且内容可控的结肠镜视频生成，用于缓解临床数据稀缺问题。
...
Method: 提出ColoDiff扩散框架：1) TimeStream模块通过跨帧标记化机制解耦时间依赖，实现复杂动态建模；2) Content-Aware模块结合噪声注入嵌入和可学习原型，实现对临床属性的精确控制；3) 采用非马尔可夫采样策略，减少90%以上步骤实现实时生成。
...
TL;DR: 提出基于小波变换的频谱差分隐私框架，通过两阶段方法在保护隐私的同时提升生成图像质量
...
Method: 提出两阶段频谱DP框架：1）在低分辨率小波系数上对自回归频谱图像分词器进行DP微调，专注于隐私敏感的低频分量；2）使用公开预训练的超分辨率模型进行高分辨率上采样，利用DP的后处理特性细化细节。

Result: 在MS-COCO和MM-CelebA-HQ数据集上的实验表明，该方法相比其他领先的DP图像框架，能生成质量更高、风格捕捉更好的图像。

Conclusion: 通过将隐私预算集中在图像全局结构（低频分量），并利用公开模型进行细节增强，该方法在隐私保护和图像质量之间取得了更好的平衡。
...
TL;DR: 提出一种基于核方法的自动细胞计数方案，跳过细胞检测直接计数，适用于小数据集且能提供预测不确定性估计


<details>
  <summary>Details</summary>
Motivation: 传统手动细胞计数耗时且需要专业训练，现有自动方法只能提供标记面积和强度信息，无法准确计数细胞数量

Method: 首先进行预处理生成多个过滤图像，然后设计非参数非线性核计数器，仅需一个超参数，能处理噪声和伪影，支持多专家意见输入

Result: 在人工和真实数据集上的实验显示非常有前景的结果，能有效计数大鼠腰椎脊髓横截面的小胶质细胞

Conclusion: 提出的核计数器方案简单易训练，适用于小数据集，能提供不确定性估计，是细胞计数任务的有效解决方案
